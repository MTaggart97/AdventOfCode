...
def two_smaller_caves_visited(caves: list[str]) -> bool:
    visited = []
    for cave in caves:
        if (cave.islower()):
            if (cave in visited):
...
while queue:
    ...
    for neighbour in graph[current_element]:
        if (neighbour != start):
            if ((neighbour.islower() and (neighbour not in path or not two_smaller_caves_visited(path))) or neighbour.isupper()):
                if (neighbour == target):
                    paths += 1
